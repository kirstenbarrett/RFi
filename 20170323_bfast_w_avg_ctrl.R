...
makePlots<-function(bId,ptsDf,distThreshold,modDfList,rsGIMMS,rsLSNDVI8d,unbSPDF,VIs,getVIs,tsToBfast,tsToReg){
  ...
  VIlist$MOD13Q1ts$brnNDVI<-VIlist$MOD13Q1ts$brnNDVI/10000
  VIlist$MOD13Q1ts$brnEVI<-VIlist$MOD13Q1ts$brnEVI/10000
  VIlist$MOD13Q1ts$NDVIminCtrl<-VIlist$MOD13Q1ts$NDVIminCtrl/10000
  VIlist$MOD13Q1ts$EVIminCtrl<-VIlist$MOD13Q1ts$EVIminCtrl/10000
  # VIlist$MCD43A4ts$brnNDVI<-VIlist$MCD43A4ts$brnNDVI/10000
  # VIlist$MCD43A4ts$NDVIminCtrl<-VIlist$MCD43A4ts$NDVIminCtrl/10000
  ...
  #VInewList<-list(MCD43A4NDVI = MCD43A4NDVI, MOD13Q1NDVI = MOD13Q1NDVI, MOD13Q1EVI= MOD13Q1EVI, LSNDVI = LSNDVI)
  VInewList<-list(MCD43A4NDVIminCtrl = MCD43A4NDVIminCtrl, MOD13Q1NDVIminCtrl = MOD13Q1NDVIminCtrl, MOD13Q1EVIminCtrl= MOD13Q1EVIminCtrl, LSNDVIminCtrl = LSNDVIminCtrl)
  ...
  for (VIname in names(VInewList)){
    ...
    #requiring more years pre-fire will exclude MODIS more often
    if(VIname == 'LSNDVI'){
      strt <- yrBrn-4
    } else{
      strt <- yrBrn-3
    ...
    if(exists('bfm')){
      if (is.na(bfm$breakpoint)==F){
        #Landsat breakpoints occur at t+1, but not for MODIS
        if(VIname == 'LSNDVI'){
          brk <- bfm$breakpoint-1
        } else{
          brk <-bfm$breakpoint
        }
 ...
 #     if(bfastdetails$magnitude < -0.05){
        yrs<-unique(as.numeric(format(time(VI),'%Y')))
        imgFil<-paste('ALL_BFAST_NO_THRESH_bfastBRK_ctrl_fix',bId,sep = '')
...
###ONLY BRN POST 2002#######
brnPtsRun<-brnPts$id
bfastOutput<-lapply(brnPtsRun, function (x) makePlots(x,ptsDf,distThreshold,modDfList,rsGIMMS,rsLSNDVI8d,unbSPDF,VIs,getVIs,tsToBfast,tsToReg))
bfastOutput<-do.call(rbind,bfastOutput)
write.csv(bfastOutput,'Bfast_output_bfastBrk_ctrl_fix.csv')
