...
rsLSNDVI8d$method<-as.character(rsLSNDVI8d$method)
...
for (id in unique(rsLSNDVI8d$newId)){
  ...
  method<-as.character(nrest$method)
  ...
  if (method == 'field'){
    ...
    if (brn == 1){
      yrBrn<-nrest$yrBrn
      addtl<-as.character(nrest$addtl)
      notes<-as.character(nrest$notes)
      logged<-nrest$Logged
    } 
  }
...
ptsDf$method<-as.character(ptsDf$method)
ptsDf$addtl<-as.character(ptsDf$addtl)
ptsDf$notes<-as.character(ptsDf$notes)
ptsDf$vegType<-as.character(ptsDf$vegType)
ptsDf$hpDist[which(is.na(ptsDf$hpDist) == F)]<-round(ptsDf$hpDist[which(is.na(ptsDf$hpDist) == F)],0)
ptsDf$baDist[which(is.na(ptsDf$baDist) == F)]<-round(ptsDf$baDist[which(is.na(ptsDf$baDist) == F)],0)
...
makePlots<-function(bId,ptsDf,distThreshold,modDfList,rsGIMMS,rsLSNDVI8d,rsLandsatSR,unbSPDF,VIs,getVIs,tsToBfast,tsToReg,order,bfasth,form,outDir){
...
#   #GET BEST BREAKPOINT
  ...
  if(minMag < 0){
   ...
   #   if (is.na(bfm$breakpoint)==F){
        ...
        if(exists('bfm')){
        try(reg <- tsreg(VI, change = brk, h = bfasth))
        if (exists('reg')){
          ...
          sitedetails<-cbind(VIname,bId,method,yrBrn,hpDist,baDist,vegType,addtl,notes,logged,round(brnLat,4),round(brnLon,4))
    #     if(bfastdetails$magnitude < -0.01){
          yrs<-unique(as.numeric(format(time(VI),'%Y')))
          imgFil<-paste('Bfast_brk_order',order,sep = '')
          imgFil<-paste(imgFil,as.character(form)[3],sep = '_')
          imgFil<-paste(imgFil,bfasth,sep = '_')
          imgFil<-paste(imgFil, '_ctrl_avg_',bId,sep = '')
          imgFil<-paste(imgFil,VIname,sep = '')
          imgFil<-paste(imgFil,'.pdf',sep = '')
          imgFil<-paste(outDir,imgFil,sep = '/')
...
brnPtsRun<-brnPts$id
orderList <- c(1,2,3)
bfasthList<-c(0.25,0.5,1)
formList<-c(response~harmon+trend,response~harmon)
bfastCombos<-expand.grid(orderList,bfasthList,formList)
names(bfastCombos)<-c('order','bfasth','form')
for (comboN in seq(1,nrow(bfastCombos))){
  combo<-bfastCombos[comboN,]
  order<-combo$order
  bfasth<-combo$bfasth
  form<-combo$form[[1]]
  outDir<-paste('Bfast_best_brk_order',order, sep = '')
  outDir<-paste(outDir,form[3],sep = '_')
  outDir<-paste(outDir,'h',sep = '_')
  outDir<-paste(outDir,bfasth,sep = '')
  dir.create(paste(direct,outDir,sep = ''))
  bfastOutput<-lapply(brnPtsRun, function (x) makePlots(x,ptsDf,distThreshold,modDfList,rsGIMMS,rsLSNDVI8d,rsLandsatSR,unbSPDF,VIs,getVIs,tsToBfast,tsToReg,order,bfasth,form,outDir))
  bfastOutput<-do.call(rbind,bfastOutput)
  outFil<-paste(direct,outDir,sep = '')
  outFil<-paste(outFil,'.csv',sep = '')
  write.csv(bfastOutput,outFil)
  rm(outDir,bfastOutput,outFil)
}
