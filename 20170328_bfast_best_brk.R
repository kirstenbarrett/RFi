...
#  GIMMSts$DayOfYear<-format(GIMMSts$YYYYMMDD,'%j')
  ...
  #GET BEST BREAKPOINT
  brk<-0 #only select negative magnitude breakpoints
 ...
 #     if(bfastdetails$magnitude < -0.01){
        ...
        imgFil<-paste('Bfast_brk_order1_harmon_trend_default_h_ctrl_avg_',bId,sep = '')
...
write.csv(bfastOutput,'Bfast_brk_order1_harmon_trend_default_h_ctrl_avg.csv')
