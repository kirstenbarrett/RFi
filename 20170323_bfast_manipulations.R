...
makePlots<-function(bId,ptsDf,distThreshold,modDfList,rsGIMMS,rsLSNDVI8d,unbSPDF,VIs,getVIs,tsToBfast,tsToReg){
  ...
  MCD43A4NDMI <- zoo(MCD43A4VIs$brnNDVI,MCD43A4VIs$bDat)
  MCD43A4NDVIminCtrl <- zoo(MCD43A4VIs$NDVIminCtrl,MCD43A4VIs$bDat)
  ...
  VInewList<-list(MCD43A4NDVI = MCD43A4NDVI, MOD13Q1NDVI = MOD13Q1NDVI, MOD13Q1EVI= MOD13Q1EVI, LSNDVI = LSNDVI,MCD43A4NDVIminCtrl = MCD43A4NDVIminCtrl, MOD13Q1NDVIminCtrl = MOD13Q1NDVIminCtrl, MOD13Q1EVIminCtrl= MOD13Q1EVIminCtrl, LSNDVIminCtrl = LSNDVIminCtrl)
  ...
  for (VIname in names(VInewList)){
    ...
    try(bfm <- bfastmonitor(bts, start = strt+2,order = 3))
    if(exists('bfm')){
      if (is.na(bfm$breakpoint)==F){
        try(reg <- tsreg(VI, change = bfm$breakpoint, h = 0.5))
      }
    }
    if (exists('reg')){
 ...
 #     if(bfastdetails$magnitude < -0.01){
        ...
        imgFil<-paste('Bfast_brk_order3_harmon_trend_default_h_ctrl_avg_',bId,sep = '')
...
write.csv(bfastOutput,'Bfast_brk_order3_harmon_trend_default_h_ctrl_avg.csv')
